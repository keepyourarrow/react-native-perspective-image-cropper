...
RCT_EXPORT_METHOD(crop:(NSDictionary *)points imageUri:(NSString *)imageUri callback:(RCTResponseSenderBlock)callback)
{
    ...
    NSLog(@"newLeft = %@", NSStringFromCGPoint(newLeft));
    NSLog(@"newRight = %@", NSStringFromCGPoint(newRight));
    NSLog(@"newBottomLeft = %@", NSStringFromCGPoint(newBottomLeft));
    NSLog(@"newBottomRight = %@", NSStringFromCGPoint(newBottomRight));
...
- (CGPoint)cartesianForPoint:(CGPoint)point height:(float)height width:(float)width {
    return CGPointMake(height - point.y, width - point.x);
}
